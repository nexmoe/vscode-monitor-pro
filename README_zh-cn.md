...
- **macOS Apple Silicon（mactop）**：可选的 [mactop](https://github.com/metaspartan/mactop) 后端带来 SoC 级指标——CPU/GPU/ANE 功率、温度与 GPU 使用率。首次运行会提示一键 `brew install mactop`。
- **GPU 监控**：通过 `nvidia-smi` 支持 NVIDIA GPU，并借助 mactop 支持 Apple Silicon 集成 GPU——使用率、温度与显存（仅 NVIDIA）图表，并提供每 GPU 阵列视图。
- **瓦特功率**：Windows 显示带符号的电池净功率，Apple Silicon 显示 SoC 总功率；0W 虚线基准与 5 点滑动平均让读数更稳定。
- **每核 CPU 视图**：CPU 使用率与温度卡片可切换为每核网格视图。
- 另有一批修复：主题自适应图表配色、macOS 27 电池健康度、最后一个 VS Code 窗口关闭时结束 mactop 等。
...
14 项可独立开关的指标，以 Codicon 图标 + 格式化数值展示：

| 指标       | 默认 | 图标                                | 示例                    |
| ---------- | ---- | ----------------------------------- | ----------------------- |
| CPU        | 开   | `$(chip)`                           | `73.2%`                 |
| 活跃内存   | 开   | `$(pie-chart)`                      | `4.21 / 15.6 GiB`       |
| 电池       | 开   | `$(plug)`                           | `85.2% (充电中)`        |
| 已用内存   | 关   | `$(pie-chart)`                      | `8.15 / 15.6 GiB`       |
...
> GPU 指标需要 NVIDIA 硬件（`nvidia-smi`），或 mactop 后端的 Apple Silicon；否则自动隐藏。GPU 显存另需 VRAM，而 mactop 的集成 GPU 后端不提供该数据。

### 资源占用视图

侧边栏专属面板，提供 14 项实时折线/柱状图：CPU、内存（活跃/已用）、网络（下行/上行）、磁盘（读取/写入）、电池、电池功率 / SoC 功率、CPU 温度、CPU 频率、GPU、GPU 温度、GPU 显存。在 macOS Apple Silicon 上使用 mactop 后端时，功率图将切换为 SoC 功率模式，显示芯片总功耗（CPU + GPU + ANE）。

每张图表包含：

- Canvas 2D 实时渲染，渐变填充与贝塞尔曲线
- 自适应 Y 轴缩放，当前最大刻度以 `↑` 前缀标注
- 折线图/柱状图一键切换
- **单核/单卡阵列视图**：CPU 使用率、CPU 温度、GPU、GPU 温度、GPU 显存卡片可切换为彩色条形网格——每个 CPU 核心（`C0`、`C1`、…）或每张显卡（`G0`、`G1`、…）对应一根条，展示各自的使用率 / 温度 / 显存。悬停 GPU 单元可查看显卡型号（GPU 显存视图还显示已用/总量）。
- 副标题：电池健康度、充放电状态、温度最低值、频率范围
- 可调节历史点数（10~500）

GPU 图表默认启用，在无 GPU 数据源（NVIDIA `nvidia-smi` 或 Apple Silicon 上的 mactop 后端）时自动隐藏。单卡阵列视图仅在两块及以上 GPU 时显示。

底部**信息区**显示运行时间、系统发行版和彩色磁盘空间进度条。这些信息卡属于 `resourceUsage.charts` 配置的一部分，可通过 `charts.osDistro.enabled` / `charts.uptime.enabled` / `charts.diskSpace.enabled` 开关（默认启用）。

### 按需查询

被关闭的指标**完全不会被查询**——采集完全由启用项驱动。底层数据源调用（macOS/Linux 上的 `systeminformation`，Windows 上的 Go 后端）只会获取当前正在使用的维度：

- 状态栏 `monitor-pro.metrics.*` 开关与 webview `resourceUsage.charts.*.enabled` 合并为同一份"已启用集合"。
...
| `pnpm run l10n:check`               | 校验 l10n bundle 与源码一致      |
| `pnpm run l10n:parity`              | 校验各语言 key 一致              |
...
- [TrafficMonitor](https://github.com/zhongyang219/TrafficMonitor)：参考其 PDH 查询生命周期（常驻句柄 + 初始基线采集 + 逐次单次采集）重构了 Windows CPU 使用率监控，使读数与任务管理器完全一致
