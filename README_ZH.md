...
Monitor Pro 是一款全面的 VS Code 资源监控工具，实时跟踪系统指标，支持本机器性能指标，并在设计之初就考虑到对于各种远程开发环境的支持性（如 Remote-SSH 和 WSL）。该扩展提供了美观易读的仪表板视图，尤其方便了在无 GUI 的系统上进行性能监控。

它采用**混合架构**：Windows 上使用原生 Go + gopsutil 后端实现最高性能，其他平台使用内置的 Node.js (`systeminformation`) 数据源。

这样做的原因是，虽然 `systeminformation` 在跨平台兼容性方面表现出色，但在 Windows 上它依赖于 PowerShell/WMI 查询来获取系统数据，这会导致严重的性能问题，尤其是在高刷新率或资源受限的环境中。通过引入 Go 后端，我们能够直接调用 OS API，显著提升数据采集速度，提供更流畅的用户体验。
...
在 Windows 上，扩展会启动一个轻量级的 Go 二进制 `monitor.exe`，通过 [gopsutil](https://github.com/shirou/gopsutil) 直接读取系统数据。这消除了 Node.js `systeminformation` 库所依赖的 PowerShell/WMI 查询开销，实现了 **10 倍以上的数据采集速度提升** —— 典型轮询周期从 约200ms 降至 20ms 以下。

Go 后端通过 HTTP（localhost）与扩展通信，使用原始 JSON 协议。如果 Go 二进制不可用或启动失败，扩展会自动回退到 `systeminformation` 数据源。

相较于同类插件，本扩展基本实现了 Windows 平台上性能的极限，提供了流畅的实时监控体验。
