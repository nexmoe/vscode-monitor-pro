...
Monitor Pro 是一款实时系统资源监控工具，可以直接在 VS Code 状态栏和专属 Webview 面板中呈现。无论本地开发、Remote SSH 还是 WSL 环境，均可提供一致的监控体验。
...
> **破坏性变更 0.7.0**
>
> 状态栏默认仅显示 CPU、活跃内存和电池三项。网络、CPU 温度、CPU 频率、运行时间、磁盘 I/O、磁盘空间、系统发行版等指标默认不再显示在状态栏上，需在 VSCode 插件设置的 `monitor-pro.metrics.*` 中手动开启。
>
> 进行该改动的理由是：随着资源占用视图的引入，状态栏显示过多指标会显得杂乱且重复。默认保留 CPU、内存和电池这三项最核心的指标，其他指标用户可以根据需要选择性开启。另外，这也避免了新用户初次安装时状态栏过于拥挤的问题，提升了默认体验。
>
> 大更新中新增的资源占用视图不受影响。
