...
Monitor Pro 是一款实时系统资源监控工具，直接在 VS Code 状态栏和专属 Webview 面板中呈现。本地、Remote SSH、WSL 均可使用，不受 GUI 环境限制。

采用**混合架构**：Windows 上使用原生 Go 二进制绕过 PowerShell/WMI 开销，实现 **10 倍以上的数据采集速度提升**；macOS 和 Linux 自动回退到 Node.js (`systeminformation`) 数据源，确保全平台兼容。

> [!WARNING]
>
> **破坏性变更 0.7.0**
>
> 状态栏默认仅显示 CPU、活跃内存和电池三项。其余指标（网络、CPU 温度、CPU 频率、运行时间、磁盘 I/O、磁盘空间、系统发行版）需在 `monitor-pro.metrics.*` 中手动开启。
>
> 背景：资源占用视图上线后，大多数指标在专属面板中可视化效果更好。状态栏保留最核心的三项指标，避免新用户初次安装时过于拥挤。
